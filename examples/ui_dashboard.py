...
class DashboardUI:
    ...
    def update_app_status(self, app_name: str, status: str, progress: float = None, 
    ...
    def _create_summary_panel(self) -> Panel:
        ...
        with self.lock:
            completed = sum(1 for app in self.app_statuses.values() if app.status == "completed")
            running = sum(1 for app in self.app_statuses.values() if app.status == "running")
            errors = sum(1 for app in self.app_statuses.values() if app.status == "error")
            pending = sum(1 for app in self.app_statuses.values() if app.status == "pending")
    ...
    def _create_app_panel(self, app: AppStatus) -> Panel:
        ...
        if app.status == "running" and app.progress > 0:
            progress_bar = f"{'█' * int(app.progress / 5)}{'░' * (20 - int(app.progress / 5))}"
    ...
    def print_final_summary(self):
        ...
        with self.lock:
            completed = [app for app in self.app_statuses.values() if app.status == "completed"]
            errors = [app for app in self.app_statuses.values() if app.status == "error"]
        ...
        for app in self.app_statuses.values():
            status_color = "green" if app.status == "completed" else "red" if app.status == "error" else "yellow"
            table.add_row(
                app.name,
        ...
        self.console.print(f"✅ Successfully Generated: {len(completed)}/{self.total_apps} apps")
        if errors:
...
class UIManager:
    ...
    def update_app_status(self, app_name: str, status: str, progress: float = None,
                         current_task: str = "", error_message: str = ""):
        """Update app status."""
        self.dashboard.update_app_status(app_name, status, progress, current_task, error_message)
