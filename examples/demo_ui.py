...
async def simulate_app_generation(ui_manager: UIManager, app_name: str):
    ...
    try:
        for i, (task, progress) in enumerate(stages):
            ui_manager.update_app_status(app_name, "running", progress, task)
        ...
        ui_manager.update_app_status(
            app_name, "completed", 100.0, f"Successfully generated {len(files)} files"
        )
...
async def demo_ui():
    ...
    try:
        ...
        for app_name in app_names:
            task = asyncio.create_task(simulate_app_generation(ui_manager, app_name))
            tasks.append(task)
