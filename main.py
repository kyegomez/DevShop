...
class CSVAppIngester:
    ...
    def validate_csv_structure(self, df: pd.DataFrame) -> bool:
        ...
        """
        required_columns = {
            'name', 'description', 'app_goal', 'target_user', 
            'main_problem', 'design_preferences'
        }
...
class MultiAppOrchestrator:
    ...
    def __init__(self, csv_file_path: str, output_directory: str = "generated_apps", max_concurrent: Optional[int] = None, show_progress: bool = True):
        """
        ...
        self.logger.info(f"Initialized with {self.max_concurrent} concurrent workers (CPU cores: {multiprocessing.cpu_count()})")
        ...
        try:
            ...
            if result.get("success", False):
                self.update_app_status(
                    app_name, 
                    "completed", 
                    f"Generated in {result['output_directory']}"
                )
    ...
    async def _generate_single_app(self, spec: AppSpecification, semaphore: asyncio.Semaphore) -> Dict[str, Any]:
        """
    ...
    def generate_all_apps(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"🚀 Starting concurrent multi-app generation from {self.csv_file_path}")
        self.logger.info(f"💻 Using {self.max_concurrent} concurrent workers (CPU cores: {multiprocessing.cpu_count()})")
        ...
        try:
            ...
            if self.show_progress:
                self.display_progress_dashboard("🚀 Initializing Concurrent App Generation")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_concurrent) as executor:
                ...
                self.logger.info(f"🔄 Submitted {len(future_to_spec)} apps for concurrent generation")
                ...
                for future in concurrent.futures.as_completed(future_to_spec):
                    ...
                    try:
                        ...
                        self.logger.error(f"❌ Exception in future for {spec.name}: {e}")
            ...
            self.logger.info(f"📈 Average time per app: {total_time/len(specifications):.2f} seconds")
...
def main():
    ...
    parser.add_argument(
        "--no-progress", 
        action="store_true",
        help="Disable progress dashboard display"
    )
    parser.add_argument(
        "--log-level", 
    ...
    try:
        ...
        max_concurrent = args.max_concurrent if args.max_concurrent is not None else optimal_workers
        ...
        print(f"📈 Avg per app: {results['total_time_seconds']/results['total_apps']:.2f} seconds")
